...
@Service
public class EnvironmentService implements AzureHandler{
    ...
    public String findFile(RepositoriesAzure repository) {
        try {
            ...
            String path = (String) json.query("/results/0/path");

            var yamlFile = 
                httpHelper.doGet(url + "/_apis/git/repositories/"+repository.getId()
                                    +"/items?api-version=7.0&path="+path);
            
            return yamlFile.body();
        } catch (Exception e) {
            return null;
        }
    }
